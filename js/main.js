window.addEventListener("load", async () => {
    ...
    const rawSchedule = data.dates[0].games;
    const schedule = rawSchedule.map(({ gamePk, teams, status }) => ({ gamePk, teams, status }));
 ...
 async function getGameData(gameObj) {
     ...
     const data = await res.json();
     const { datetime, probablePitchers, teams } = data.gameData;
 
     return { datetime, probablePitchers, teams };
 }
